<template>
  <div>
    ...
    <form v-else @submit.prevent="submitArticle" class="max-w-4xl">
      <!-- Titre -->
      <div class="mb-6">
      ...
      <div class="mb-6">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-indigo-500 transition">
          ...
          >
            ...
            <p class="text-gray-600">Cliquez pour remplacer l'image</p>
            <p v-if="form.cover_image" class="text-sm text-green-600 mt-2">
              ✓ Nouvelle image sélectionnée: {{ form.cover_image.name }}
            </p>
          </button>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        <button
          ...
          class="btn-primary"
        >
          <span v-if="!articleStore.isLoading">Enregistrer les modifications</span>
...
const submitArticle = async () => {
  ...
  const result = await articleStore.updateArticle(id, form)

  if (result) {
    success('Article modifié avec succès!')
    router.push(`/articles/${id}`)
  } else {
    error(articleStore.error || 'Erreur lors de la modification')
  }
}
...
onMounted(async () => {
  ...
  if (article.value) {
    ...
    if (!authStore.isAdmin && article.value.user_id !== authStore.currentUser?.id) {
    ...
    form.status = article.value.status
  }
})
